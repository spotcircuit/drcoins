    ...
    {uploadedImages.length === 0 ? (
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {uploadedImages.map((src, i) => (
          <div
            key={src}
            className="group relative overflow-hidden rounded-2xl border border-white/15 bg-white/5 shadow-sm"
          >
...
const ProductMock: React.FC = () => (
  <Container className="py-10 lg:py-12">
    <div className="grid gap-8 lg:grid-cols-2">
      <div className="relative aspect-square overflow-hidden rounded-2xl bg-gradient-to-br from-purple-100 to-slate-100">
        <div className="absolute inset-0 grid place-items-center">
          <div className="text-sm font-medium text-purple-600">Product Image</div>
        </div>
      </div>
      <div>
        <Pill>Official Reseller</Pill>
        <h1 className="mt-4 text-3xl font-bold tracking-tight text-white">LiveMe Coins Bundle</h1>
        <p className="mt-3 text-slate-300">Get instant delivery of LiveMe coins to your account. Secure, fast, and reliable.</p>
      ...
      <div className="lg:col-span-2">
        <SectionCard title="Billing Information">
          <div className="space-y-4">
            ...
            <input type="text" placeholder="Card Number" className="w-full rounded-lg border px-3 py-2" />
          </div>
        </SectionCard>
      </div>
      <div>
    ...
    <div className="grid gap-6 md:grid-cols-3">
      {["Orders", "Users", "Products"].map((section) => (
        <SectionCard key={section} title={section}>
          ...
          <div className="text-sm text-slate-600">Total {section.toLowerCase()}</div>
        </SectionCard>
      ))}
    </div>
  </Container>
        ...
        <div className="relative grid gap-10 md:grid-cols-2">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <button onClick={()=>setPage("Shop Points")} className="inline-flex items-center justify-center rounded-xl border border-purple-600 bg-white px-4 py-2 text-sm font-semibold text-purple-700 shadow-sm hover:bg-purple-50">
                Shop Points
              </button>
            </div>
          ...
          <div className="">
            <div className="relative isolate overflow-hidden rounded-2xl border border-purple-200 bg-gradient-to-br from-purple-50 to-white p-6 shadow-md">
              <div className="mb-4 text-sm font-semibold text-gray-900">Featured bundles</div>
              <div className="grid gap-3 sm:grid-cols-2">
                {[{name:"5,460 Coins", price:69.99},{name:"36,400 Coins", price:399.99},{name:"Nobility: Duke", price:149.99},{name:"Nobility: King", price:599.99}].map((p)=> (
                  <div key={p.name} className="rounded-xl border border-purple-200 bg-white p-4 shadow-sm">
                    <div className="text-sm font-medium text-gray-900">{p.name}</div>
                    <div className="mt-1 text-xs text-slate-600">Instant delivery after ID match</div>
                    <div className="mt-3 flex items-center justify-between">
                      <div className="text-lg font-semibold text-gray-900">${p.price.toFixed(2)}</div>
                      <button onClick={()=>setPage(p.name.includes("Coins")?"Shop Coins":"Shop Points")} className="rounded-lg border border-purple-200 bg-purple-50 px-3 py-1.5 text-xs font-semibold text-purple-700 hover:bg-purple-100">View</button>
                    </div>
                  </div>
                ))}
              </div>
